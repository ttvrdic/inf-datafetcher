...
for country in opts.countries:
		...
		call(["python","GetEurostatDataSingleCategory.py","-country",country,"-categories",detail_category,"-st_period",opts.st_period[0],"-end_period",opts.end_period[0]])
                totalPerc += percStep
                end = time.time()
                elapsed = end - start
...
for country in opts.countries:
	...
	for category in detail_categories:
		call(["python","GetEurostatData.py","-country",country,"-categories",category,"-st_period",opts.st_period[0],"-end_period",opts.end_period[0]])
		totalPerc += percStep	
		end = time.time()
		elapsed = end - start
		if step_calculated == 0 :
...
for country in opts.countries:
	for comp_country in opts.countries:
		if(comp_country !=country):
			call(["python","GetEurostatDataDiff.py","-countries",country,comp_country,"-categories","NATURAL","-st_period",opts.st_period[0],"-end_period",opts.end_period[0]])
			totalPerc += percStep
			end = time.time()
                	elapsed = end - start	
			if step_calculated == 0 :
