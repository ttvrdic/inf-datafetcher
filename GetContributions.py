...
# This script downloads all contribution data 
# 4 main aggregates, 12 subaggregates, 12 main groups
		...
		data_list=[]
			...
			if (value == 'NaN' or value == 'nan'):
				...
				# this is why, in order to get value from last year, we have to go 12 months "ahead"
					...
					#TODO revise if problems with some countries
					# also, added the same value condition, since we want to ignore the flash estimate 
					# if the values are same, which also leads to current value of 0
					if(a!=0 or value !=0 or float(current_value)==float(value_last_year)): 
